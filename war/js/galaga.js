...
var BULLETHEIGHT = 8;
var BULLETWIDTH = 2;
var GUYOFFSET = GUYWIDTH / 2; // player sprite positioned a mouse in center of
// the image
var PLAYERLIVES = 5;
// Game counters
var playerScore = 0;
...
jQuery(document).ready(function($) {
	...
	/*
	 * jQuery("#classicOrFree").dialog({ resizable: false, height: 140, modal:
	 * true, buttons: { "Classic": function() { gameTypeClassic = true; ready();
	 * jQuery(this).dialog("close"); }, "Free-Flight": function() {
	...
	$GALAGA_CANVAS.on('click', function(e) {
		...
		if (intervalLoop == 0) {
			...
			if (bullets.length < maxBulletsNum) {
				if (isGalagaMerged) {
					bullets.push(new Bullet(player.x - 1 - GUYOFFSET, player.y
							- GUYOFFSET, BULLETHEIGHT, BULLETWIDTH, 0));
					bullets.push(new Bullet(player.x + 1 + GUYOFFSET, player.y
							- GUYOFFSET, BULLETHEIGHT, BULLETWIDTH, 0));
				} else {
					bullets.push(new Bullet(player.x - 1, player.y - GUYOFFSET,
							BULLETHEIGHT, BULLETWIDTH, 0));
				}
				sound11.play();
			}
		}
...
function doKeyAction() {
	...
	if (pressedKeys[32]) { // space
		...
		if (luckyLife || (bullets.length == 0 && barrierBullets.length == 0)) {
			bullets.push(new Bullet(player.x - 15, player.y - GUYOFFSET,
					BULLETHEIGHT, BULLETWIDTH, -1));
			bullets.push(new Bullet(player.x - 5, player.y - GUYOFFSET,
					BULLETHEIGHT, BULLETWIDTH, -0.5));
			bullets.push(new Bullet(player.x + 5, player.y - GUYOFFSET,
					BULLETHEIGHT, BULLETWIDTH, 0.5));
			bullets.push(new Bullet(player.x + 15, player.y - GUYOFFSET,
					BULLETHEIGHT, BULLETWIDTH, 1));
		...
		if (luckyLife || (bullets.length == 0 && barrierBullets.length == 0)) {
			bullets.push(new Bullet(player.x + 15, player.y - GUYOFFSET,
					BULLETHEIGHT, BULLETWIDTH, 0, "bomb"));
		}

	} else if (pressedKeys[67]) { // C - Barrier
		if (luckyLife || (barrierBullets.length == 0)) {
			barrierBullet = new Bullet(player.x - BULLETWIDTH * 20, player.y
					- GUYOFFSET - 30, BULLETHEIGHT, BULLETWIDTH * 40);
...
/**
 * clears the GALAGA_CANVAS and calls a bunch of helper methods to redraw and
 * check collisions. This method is where most of the "heavy lifting" is done
 */
	...
	if (!isSpiderMove) {
		...
		if (!isViming) {
			...
			if (isCaptured) {
				...
				if (tempVimImgY >= 0) {
					sound3.play();
					GALAGA_CONTEXT.drawImage(vim, spider.x - 24, spider.y + 20,
							70, tempVimImgY);
					tempVimImgY -= 1;
				}
			} else {
...
function standByGalaga() {
	...
	if (clonePlayer.x < (player.x + player.width / 2)) { // player is on the
		// right
		if (!stopThere) {
			clonePlayer.x -= 2;
		}
		...
		if (clonePlayer.y >= player.y
				&& player.x >= (clonePlayer.x + clonePlayer.width)) {
			completeStandBy();
			stopThere = false;
		}
	} else if ((player.x + player.width / 2) < clonePlayer.x) { // player is on
		// the left
		if (!stopThere) {
			clonePlayer.x += 2;
		}
		...
		if (clonePlayer.y >= player.y
				&& (player.x + player.width) <= clonePlayer.x) {
			completeStandBy();
			stopThere = false;
		}
	} else {
					...
					function(index, badGuy) {
						...
						if (Math.floor(Math.random() * FIRECHANCEDENOMINATOR) > FIRECHANCENUMERATOR) {
							if (badGuy instanceof Boss) {
								...
								var badBullet = new Bullet(badGuy.x
										+ (badGuy.width / 2), badGuy.bottom(),
										BULLETHEIGHT, BULLETWIDTH, 0);
...
function redrawPlayerGalaga(str) {
	if (!isCapturing && !isGalagaMerging) {
		...
		else if (mouse.x > 400 - player.width)
	...
	if (isCapturing) {
		tmpYPos -= 2;
		GALAGA_CONTEXT.drawImage(player.img, spider.x, tmpYPos - GUYOFFSET,
		...
		GALAGA_CONTEXT.drawImage(player.img, player.x - GUYOFFSET, player.y
				- GUYOFFSET, player.height, player.width);
		// if(!clonePlayer){
		// clonePlayer = new Guy(spider.x, spider.y-(spider.height/2), good, 0,
		// GUYWIDTH, GUYHEIGHT, 0, 5);
		// }
		GALAGA_CONTEXT.drawImage(player.img, clonePlayer.x - GUYOFFSET,
				clonePlayer.y - GUYOFFSET, clonePlayer.height,
				clonePlayer.width);
	} else if (isGalagaMerged) {
		GALAGA_CONTEXT.drawImage(player.img, player.x - GUYOFFSET * 2, player.y
				- GUYOFFSET, player.height, player.width);
		GALAGA_CONTEXT.drawImage(player.img, player.x, player.y - GUYOFFSET,
				player.height, player.width);
	} else {
		GALAGA_CONTEXT.drawImage(player.img, player.x - GUYOFFSET, player.y
				- GUYOFFSET, player.height, player.width);
	...
	GALAGA_CONTEXT.fillText("PlayerScore: [" + playerScore + "] Level: ["
			+ level + "] " + str, 20, 390);
	...
	if (player != undefined && player.x != undefined) {
		jQuery.each(badGuys, function(index, badGuy) {
			if (intersect(badGuy)) {
				GALAGA_CONTEXT.drawImage(explosion, player.x - GUYOFFSET,
						player.y - GUYOFFSET, 32, 32);
				sound7.play();
				if (isGalagaMerged) {
					numOfGalaga--;
	...
	jQuery.each(badGuys, function(indexGuy, badGuy) {
		if (badGuy instanceof Boss) {
			if (badGuy.isFiredLaser
					&& (badGuy.getLaserHeight() + badGuy.top()) > 380) {
				if (badGuy.left() + 15 < player.right()
						&& badGuy.left() + 55 > player.left()) {
					if (isGalagaMerged) {
						numOfGalaga--;
						isGalagaMerged = false;
					} else {
		...
		jQuery.each(bullets, function(indexBullet, bullet) {
			if ((bullet != undefined && badGuy != undefined)
					&& intersectOther(badGuy, bullet)) {
				if (bullet.bulletType == "bomb") {
					bullets.push(new Bullet(bullet.x, bullet.y,
							BULLETHEIGHT * 5, BULLETWIDTH * 30, 0, "bomb"));
				}
				if (badGuy.hp == 1) {
					badGuy.hp--;
					...
					if (isCaptured && badGuy.isSpider) {
						...
						isCaptured = false;
						if (!clonePlayer) {
							clonePlayer = new Guy(spider.x, spider.y
									- (spider.height / 2), good, 0, GUYWIDTH,
									GUYHEIGHT, 0, 5);
						}
						if (clonePlayer) {
					...
					} else if (!isCaptured && badGuy.isSpider
							&& (isSpiderCount <= isSpiderMaxCount)) {
						selectSpider();
					}
					badGuys.splice(indexGuy, 1);
					GALAGA_CONTEXT.drawImage(explosion, badGuy.x, badGuy.y, 32,
							32);
...
function moveBadGuys() {
	...
	jQuery.each(badGuys, function(index, badGuy) {
		vel = badGuy.vel;
		if ((badGuy.x + badGuy.width) > $GALAGA_CANVAS.width()
				&& badGuy.direction) { // moving right hit a wall?
			isWall = true;
			badGuy.x += badGuy.velocity;
		} else if (badGuy.x < 0 && !badGuy.direction) { // moving left hit a
			// wall?
			isWall = true;
			badGuy.x -= badGuy.velocity;
		} else { // no walls GO
		...
		if (badGuy.isSpider) {
			...
			if (isCaptured) {
				GALAGA_CONTEXT.drawImage(player.img, badGuy.x, badGuy.y
						- GUYOFFSET * 2, player.height, player.width);
			}
		}
	});
		...
		jQuery.each(badGuys, function(index, badGuy) {
			...
			if (DEBUG) {
				console.log("BadGuy Bottom [" + badGuy.bottom()
						+ "] Canvas bottom [" + GALAGA_CANVAS.height + "]");
			}
...
function redrawBadGuys() {
	jQuery.each(badGuys, function(index, badGuy) {
		if (DEBUG) {
			GALAGA_CONTEXT.fillStyle = "White";
			GALAGA_CONTEXT.fillText("HP: [" + badGuy.hp + "]", badGuy.x,
					badGuy.y - 2);
		}
		GALAGA_CONTEXT.drawImage(badGuy.img, badGuy.x, badGuy.y, badGuy.width,
				badGuy.height);
	});
}
...
function redrawBullets() {
	...
	jQuery.each(bullets, function(index, bullet) {
		if (bullet != undefined) {
			...
			GALAGA_CONTEXT.fillStyle = "White";
			GALAGA_CONTEXT.fillRect(bullet.x, bullet.y, bullet.width,
					bullet.height); // X, Y, width, height
			if (bullet.y < 0) {
	...
	jQuery.each(badBullets, function(index, badBullet) {
		if (badBullet != undefined) {
			...
			GALAGA_CONTEXT.fillRect(badBullet.x, badBullet.y, badBullet.width,
					badBullet.height); // X, Y, width, height
		...
		if (badGuy instanceof Boss) {
			if (badGuy.isFiredLaser) {
				...
				if (endLaser >= 2000) {
					...
					for (start = startLaser; start <= endLaser; start++) {
						GALAGA_CONTEXT.drawImage(laser, badGuy.x - GUYOFFSET
								+ 15, start - GUYOFFSET + 60, 40, 24);
					}
				}
			...
			jQuery.each(badGuy.suriArr, function(index, suri) {
				try {
					...
					if (suri.right() < 0 || suri.left() > 400
							|| suri.top() > 400) {
						badGuy.suriArr.splice(index, 1);
						console.log("suri removed->" + index);
					} else {
						// console.log("suri
						// draw->"+index+"["+suri.left()+","+suri.top()+"]");
						GALAGA_CONTEXT.drawImage(suri.img, suri.left(), suri
								.top(), 15, 15);
					}
				} catch (e) {
	...
	jQuery.each(barrierBullets, function(index, barrierBullet) {
		if (barrierBullet != undefined) {
			...
			GALAGA_CONTEXT.fillRect(barrierBullet.x, barrierBullet.y,
					barrierBullet.width, barrierBullet.height); // X, Y, width,
			// height
		}
	});
}
...
/**
 * player has been killed or got a game over optional param str of reason WHY
 * the game ended
 */
...
function intersect(obj) {
	if (DEBUG) {
		// console.log("Top [" + obj.top() + "] Bottom [" + obj.bottom() + "]
		// Right [" + obj.right() + "] Left [" + obj.left() + "]");
	}
	if (isGalagaMerged)
		// GALAGA_CONTEXT.drawImage(player.img, player.x - GUYOFFSET*2, player.y
		// - GUYOFFSET, player.height, player.width);
		// GALAGA_CONTEXT.drawImage(player.img, player.x, player.y - GUYOFFSET,
		// player.height, player.width);
		return (obj.left() >= player.x - player.width
				&& player.x + player.width >= obj.right()
				&& obj.top() <= player.y + GUYOFFSET && player.y <= obj
				.bottom());
	return (obj.left() <= player.x + GUYOFFSET && player.x <= obj.right()
			&& obj.top() <= player.y + GUYOFFSET && player.y <= obj.bottom());
}
